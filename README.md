...
Das Skript [`rust-projekte/md2mw.py`](rust-projekte/md2mw.py) wandelt alle Markdown-Dateien
aus `rust-projekte/src/` in eine MediaWiki-XML-Importdatei um (Export-Format 0.11).
Jeder Seite wird dabei als Kopf und Fuß der KI-Transparenzhinweis gemäß Art. 50 EU AI Act beigefügt.

```bash
# thorsten.xml im Repo-Wurzelverzeichnis erzeugen
python3 rust-projekte/md2mw.py
```

Die erzeugte `thorsten.xml` ist per `.gitignore` ausgenommen. Import in ein MediaWiki:

```bash
php maintenance/run.php importDump --uploads=false thorsten.xml
php maintenance/run.php rebuildrecentchanges
```

Alternativ über die Weboberfläche: **Spezial:Importieren**.
